    ...
    const configSchema = [
        {
            ...
            settings: [
                ...
                },
                {
                    ...
                    description: 'The delay in milliseconds before sending a nick alert.',
                    displayLabel: 'Delay',
                    values: [
                        { text: '0ms', value: 0 },
            ...
            settings: [
                {
                    ...
                    text: ['OFF', 'ON'],
                    description: 'Alerts for players who are nicked but could not be linked to a real name.'
                }
            ]
        },
        {
            ...
            settings: [
                {
                    type: 'toggle',
                    key: 'addNicksToCubelify.enabled',
                    text: ['OFF', 'ON'],
                    description: 'Sends a message that cubelify will read to manually add denicked players to your overlay.'
                }
            ]
        }
...
class Denicker {
    ...
    setNickDisplayName(uuid, nickName, realName) {
    ...
    sendAlert(playerName, realName) {
        ...
        const sendMessage = () => {
            ...
            this.api.chat(`${this.PLUGIN_PREFIX} ${alertMsg}`);
    ...
    sendCubelifyMessage(realName) {
        if (!this.api.config.get('addNicksToCubelify.enabled')) {
            return;
        }
